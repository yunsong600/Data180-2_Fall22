...
### load United States state map data
```{r}
install.packages('map') # you only need to do this once. maps package includes various maps that we can use.
install.packages('s') # you only need to do this once
library(ggplot2)
library(maps)     # Provides latitude and longitude data for various maps
library(sf)
...
ggplot() + geom_polygon(data=states,fill=states$group,color ='black', aes(x=long,y=lat,group=group)) 
+ coord_sf(st_crs(4326))#ase is a x and y variable (x should be longitude, y= latitude ) 
```

```{r}
Map of U.S. states using ggplot2
ggplot() + geom_polygon(data=states,fill=states$group, coord_sf(st_crs(4326)), aes(x=long,y=lat,group=group),fill=region) + theme(legend.position = 'none') + coord_sf(st_crs(4326)) #ase is a x and y variable (x should be longitude, y= latitude ) 
```
```{r}
...
df_states <- st_read("/Users/yun-yeosong/Downloads/us-states (1)/us-states.shp"))
mapview(df_states, zcol='population') #
/Users/yun-yeosong/Downloads/us-states (1)/us-states.shp
```
...
#mapviewOptions(basemaps = c("OpenStreetMap"))
#map for PA only
map1 <- mapview(df_states[df_states$Name=='Pennsylvania',],layer.name='PA',col.regions=blues9)

#map for SC only
map2 <- mapview(df_states[df_states$Name=='South Carolina',],layer.name='SC',col.regions=blues9)
sync(map1,map2) 
```
```{r}
#Map of U.S. states using ggplot2
ggplot() + geom_polygon(data=states,fill='lightblue',colors ='black', aes(x=long,y=lat,group=group)) 
+ coord_sf(st_crs(4326))#ase is a x and y variable (x should be longitude, y= latitude ) 
```
```{r}
#Map of U.S. states using ggplot2
