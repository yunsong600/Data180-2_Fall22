...
```{r}
library(ggplot2)
ggplot(data=Cars93, aes(x=Origin, fill = AirBags)) + geom_bar( position = 'dodge') +labs(x="AirBags",y="Frequency",aes(x=AirBags)) 
```
...
library(mapview)
library(sf)
ggplot(au_states) + geom_sf(data=au_states,fill='lightblue') + coord_sf(crs = st_crs(3112)) 
```
